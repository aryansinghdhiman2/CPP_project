...
def admin_window():
   ...
   root.withdraw()
   l1=Label(admin_win,image=bg1)
   l1.place(x=0,y=0,relheight=1,relwidth=1)
   admin_win.geometry("1000x663")
   admin_win.resizable(False,False)
   admin_win.title("Clubs")
   def func():
      admin_win.destroy()
      root.deiconify()
   main_frame= Frame(admin_win,bg="pale turquoise",height=300,width=700,borderwidth=1,relief=RIDGE)
   main_frame.pack(pady=130)
   main_frame.pack_propagate(0)
   heading= Label(main_frame,text="Admin Menu",font=myFont, padx=15,pady=25,bg="pale turquoise")
   heading.pack()
   btn_frame= Frame(main_frame,bg="pale turquoise")
   btn_frame.pack(pady=20)
   clb_inf_btn= Button(btn_frame,text=" Club Information",font=myFont1,relief=SOLID,borderwidth=1,height=1,width=14,activebackground='light sea green',bg='pale turquoise')
   clb_inf_btn.grid(row=0, column=0,padx=4,pady=4)
   clb_rec_btn= Button(btn_frame,text="Club Recruitment",font=myFont1,relief=SOLID,borderwidth=1,height=1,width=14,activebackground='light sea green',bg='pale turquoise')
   clb_rec_btn.grid(row=0, column=1)
   clb_meet_btn= Button(btn_frame,text= "Club Meeting",font=myFont1,relief=SOLID,borderwidth=1,height=1,width=14,activebackground='light sea green',bg='pale turquoise')
   clb_meet_btn.grid(row=0, column=2,padx=4)
   clb_event_btn=Button(btn_frame,text="Club Event",font=myFont1,relief=SOLID,borderwidth=1,height=1,width=14,activebackground='light sea green',bg='pale turquoise')
   clb_event_btn.grid(row=1,column=0,columnspan=2,padx=0)
   return_btn= Button(btn_frame,text= "Return",font=myFont1,relief=SOLID,borderwidth=1,height=1,width=14,activebackground='light sea green',bg='pale turquoise',command=func)
   return_btn.grid(row=1, column=1,columnspan=2)
   clb_inf_btn.bind('<Enter>', on_enter)
   clb_inf_btn.bind('<Leave>', on_leave)
   clb_rec_btn.bind('<Enter>', on_enter)
   clb_rec_btn.bind('<Leave>', on_leave)
   clb_meet_btn.bind('<Enter>', on_enter)
   clb_meet_btn.bind('<Leave>', on_leave)
   clb_event_btn.bind('<Enter>', on_enter)
   clb_event_btn.bind('<Leave>', on_leave)
...
admin_btn= Button(btn_frame,text= "Admin",font=myFont1,relief=SOLID,borderwidth=1,height=1,width=12,activebackground='light sea green',bg='pale turquoise',command=admin_window)
